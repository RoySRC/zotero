...
preferences-sync-fileSyncing-library-metadata-sync = Sync metadata through this WebDAV profile
preferences-sync-fileSyncing-library-metadata-migrate-confirm = Prepare “{ $name }” to sync metadata and stored files through the assigned WebDAV profile? Local metadata and existing local files will be uploaded to WebDAV on the next sync. Existing zotero.org storage is not deleted.
preferences-sync-fileSyncing-library-metadata-migrated = Library “{ $name }” prepared for WebDAV metadata and file sync.
...
preferences-sync-fileSyncing-webDAVProjects-description = Create local project libraries whose files and metadata sync through WebDAV instead of zotero.org group storage. These projects use WebDAV credentials for access control.
